# -*- coding: utf-8 -*-
"""
證交所~每日信用交易統計、融資融券彙總~每日收盤CSV檔讀取並寫入資料庫
...
def READ_CSV(arg_date):
	...
	print("讀取 " + arg_date + " 證交所~每日信用交易統計、融資融券彙總CSV資料檔.")
	file_name = "./daily_mtss_data/" + arg_date + ".csv"
	...
	try:
		...
		print("$$$ Err:" + arg_date + " 證交所~每日信用交易統計、融資融券彙總CSV資料讀取異常. $$$")
		print(e)
		file.write("$$$ Err:" + arg_date + " 證交所~每日信用交易統計、融資融券彙總CSV資料讀取異常. $$$")
		file.write(e)
		file.write("\n")
		err_flag = True
	...
	df.columns = ['股票代號', '股票名稱', '融資買進', '融資賣出', '融資現金償還', '融資前日餘額', '融資今日餘額', '融資限額', '融券買進', '融券賣出', '融券現金償還', '融券前日餘額', '融券今日餘額', '融券限額', '資券互抵', '註記']
	df2 = df.loc[:,['股票代號', '股票名稱', '融資前日餘額', '融資今日餘額', '融資限額', '融券前日餘額', '融券今日餘額', '融券限額']]
	#print(df2)
...
def STORE_DB(arg_df, arg_date):
	...
	for i in range(0,len(arg_df)):
		...
		prog_last_maint = "READ_DAILY_MTSS_CSV"
	...
	if commit_flag == True:
		conn.commit()
		print(quo_date + " 證交所~每日信用交易統計、融資融券彙總，寫入/更新成功.\n")
		file.write(quo_date + " 證交所~每日信用交易統計、融資融券彙總，寫入/更新成功.\n")
	else:
		conn.execute("rollback")
		print(quo_date + " 證交所~每日信用交易統計、融資融券彙總，寫入/更新失敗Rollback.\n")
		file.write(quo_date + " 證交所~每日信用交易統計、融資融券彙總，寫入/更新失敗Rollback.\n")
	
	#關閉資料庫連線
	conn.close

def MAIN_READ_DAILY_MTSS_CSV():
	global err_flag
	global file
	err_flag = False
	...
	name = "READ_DAILY_MTSS_CSV_LOG_" + str_date + ".txt"
	file = open(name, 'a', encoding = 'UTF-8')

	print_dt = str(str_date) + (' ' * 22)
	print("##############################################")
	print("##   證交所~每日信用交易統計、融資融券彙總  ##")
	print("##              CSV檔寫入資料庫             ##")
	print("##                                          ##")
	...
	print("\n\n證交所~每日信用交易統計、融資融券彙總，每日收盤CSV檔資料庫處理結束...\n\n\n")

if __name__ == '__main__':
	MAIN_READ_DAILY_MTSS_CSV()