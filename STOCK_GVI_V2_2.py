...
def store_db(arg_df, arg_date):
	...
	for i in range(0,len(arg_df)):
		...
		try:
			...
			err_flag = True
			print(sqlstr + "\n")
			print("process STOCK_GVI error er=\n" + er.args[0] + "\n")
			file.write(sqlstr + "\n")
			file.write("process STOCK_GVI error er=\n" + er.args[0] + "\n")
...
def MAIN_STOCK_GVI():
	...
	name = "STOCK_GVI_LOG_" + str_date + ".txt"
	...
	file.write("結轉日期區間:" + str_date + "\n")

	#建立資料庫連線
	conn = sqlite3.connect("market_price.sqlite")

	sqlstr  = "SELECT SEAR_COMP_ID, COMP_ID, COMP_NAME, STOCK_TYPE "
	sqlstr += "FROM STOCK_COMP_LIST "
	sqlstr += "ORDER BY STOCK_TYPE, SEAR_COMP_ID "
	#sqlstr += "limit 1 "
	...
	if re_len > 0:
		...
		for row in result:
			...
			sqlstr  = "select CLOSE from STOCK_QUO "
			sqlstr += "where "
			sqlstr += "QUO_DATE = '" + str_date + "' and "
			sqlstr += "SEAR_COMP_ID = '" + sear_comp_id + "' "
			...
			#GVI指標計算
			#參考STOCK AI網站作法，若單季ROE < 0, GVI = (每股參考淨值/收盤價)
			if sroe < 0:
				if price != 0:
					gvi = bvps / price
				else:
		...
		df = df.sort_values(by=['GVI'], ascending=[False])[0:101]
		#print(df)
