...
def READ_CSV(arg_date):
	...
	df2 = df.loc[:,['證券代號', '證券名稱', '發行股數', '外資及陸資尚可投資股數', '全體外資及陸資持有股數', '外資及陸資尚可投資比率', '全體外資及陸資持股比率', '外資及陸資共用法令投資上限比率', '陸資法令投資上限比率', '最近一次上市公司申報外資及陸資持股異動日期']]
	#print(df2)
...
def STORE_DB(arg_df, arg_date):
	...
	for i in range(0,len(arg_df)):
		...
		last_claim_date = str(arg_df.loc[i]['最近一次上市公司申報外資及陸資持股異動日期'].replace("/","").strip())
		...
		prog_last_maint = "READ_DAILY_FR_HOLDING_SHARES_CSV"
