...
@Note: 
	上市櫃股票，6、18日均線糾結選股(當天大漲)
	並加上以上過濾條件
	A. 2條均線變異數介於0~1間(變異數值太大，線就離太遠了)
	B. 近六天成交量平均量成長30%以上，且日均量500張以上
	C. 當天收盤漲幅3%以上(當天上漲3%以上)
	D. 最近一交易日，收盤價在50MA之上
...
def Patt_Recon(arg_stock, str_prev_date, str_today):
	...
	if none_flag == False:
		...
		#6MA、18MA線近六天的數值，全部丟到一個list下，一起計算變異數
		ls_ma = []
		ls_ma.extend(df['ma6'].tail(6))
		ls_ma.extend(df['ma18'].tail(6))
		var_val = statistics.variance(ls_ma)

		#2條均線(6MA、18MA)變異數介於0~1間、最近一天成交量相對平均量成長30%、紅K棒實體3%以上(或當天上漲3%以上)、
...
def MAIN_STOCK_SELECT_TYPE07():
	...
	name = "STOCK_SELECT_TYPE07_" + str_date + ".txt"
	...
	#結果寫入EXCEL檔
	file_name = 'STOCK_SELECT_TYPE07_' + str_date + '.xlsx'
