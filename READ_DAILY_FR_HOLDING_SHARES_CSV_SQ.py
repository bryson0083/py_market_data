...
def STORE_DB(arg_df, arg_date):
	...
	if commit_flag == True:
		conn.commit()
		print(quo_date + " 櫃買中心外資及陸資投資持股統計，寫入/更新成功.\n")
		file.write(quo_date + " 櫃買中心外資及陸資投資持股統計，寫入/更新成功.\n")
	else:
		conn.execute("rollback")
		print(quo_date + " 櫃買中心外資及陸資投資持股統計，寫入/更新失敗Rollback.\n")
		file.write(quo_date + " 櫃買中心外資及陸資投資持股統計，寫入/更新失敗Rollback.\n")
	
	#關閉資料庫連線
	conn.close

def MAIN_READ_DAILY_FR_HOLDING_SHARES_CSV_SQ():
	global err_flag
	global file
	err_flag = False
	...
	print("\n\n櫃買中心外資及陸資投資持股統計，每日收盤CSV檔資料庫處理結束...\n\n\n")

if __name__ == '__main__':
	MAIN_READ_DAILY_FR_HOLDING_SHARES_CSV_SQ()